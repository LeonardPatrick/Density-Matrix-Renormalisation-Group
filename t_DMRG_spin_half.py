...
dim = 2 # Dimension of Hilbert space associated wiht a single site 
...
for j in range (0, len(M) ):
    ...
    for i in range(1,int(It2) + 1):  # + 1 included because you are using "range", which starts at 1 and ends at int(It2)
        ...
        DR = HRMAT[-i-1].shape[0]/2 # Refer to p. 1716; number of rows divided by 2 



        if j != 0:

            DR = HRMAT[-i -  (int(It4) + 1)].shape[0]/2   # Refer to p. 1763 as to why one has (int(It4) +  1)
...
# i == int(It2) signifies that the right system is now only composed of two sites. At this stage the right side becomes the system. In this current form of the code, this means that the superblock Hamilto      nian must be rearranged. Refer to p. 1700
        ...
        if i == int(It2):
                m = 4       # When dealing with spin - 1 system, this should be 9. In general, you could have dim**2 
        else:
                m = M[j]
        ...
        # Comment out if final configuration should be 00--------
        #if j == ( len(M) - 1) and i == It2 :
        #        break
        ...
        if j == 0 and  i == It2: # This is condition for wavefunction transformation to begin; it corresponds to configuration ------------------00
            ...
            psi_a = psi.reshape((-1, DL * 2), order="C")  # since the left hand side is the ennvironment dimension, DL is chosen here. AT THE MOMENT, I CANNOT EXPLAIN THIS PART
            psi_b = U.conjugate().transpose().dot(psi_a)
            psi_c = psi_b.reshape((-1, DL, 2), order="C").transpose(0, 2, 1)
            ...
            if i == int (It2):
                ...
                psi_a = psi.reshape((-1, DL * 2), order="C") 
                psi_b = U.conjugate().transpose().dot(psi_a)
                psi_c = psi_b.reshape((-1, DL, 2), order="C").transpose(0, 2, 1)
                psi_d = psi_c.reshape((-1, DL), order="C")
                psi_trial = UENVL[-1].dot(psi_d.transpose()).transpose().reshape((-1, 1))   
                ...
                psi_a = psi.reshape((-1, DR * 2), order="C")  # since the left hand side is the ennvironment dimension, DL is chosen here. AT THE MOMENT, I CANNOT EXPLAIN THIS PART
                psi_b = U.conjugate().transpose().dot(psi_a)
                psi_c = psi_b.reshape((-1, DR, 2), order="C").transpose(0, 2, 1)
    ...
    for i in range(1, It3 + 1): # + 1 included to ensure that for loop goes through full range   
            ...
            DL = HLMAT[-i-1].shape[0]/2 # Refer to p. 1716; number of rows divided by 2 
           ...
           # Construct reduced density matrix
            ...
            if HRMAT[i].shape[0] < 2* M[j]:   # it is 2*M instead of M beacuse you wish to compute 16x16 reduced density matrix before truncaiton
                        sd = HRMAT[i].shape[0]  
            elif i == int(It3):
                        sd = dim**2
            else:
           ...
           #  Uncomment if final configuration should be 00--------    
            if j == ( len(M) - 1) and i == It3 :
                break
            ...
            if j == 0 and i < 7:    # This represents generating psi_trial up to approximately the middle of the chain. I am not sure why one does this in the simpe dmrg code. However, when one is on the second sweep, this apparently is no longer applicable. NOT SURE WHY THIS IS.
                ...
                psi_a = psi.reshape((-1, DR * 2), order="C") 
                psi_b = U.conjugate().transpose().dot(psi_a)
                psi_c = psi_b.reshape((-1, DR, 2), order="C").transpose(0, 2, 1)
                psi_d = psi_c.reshape((-1, DR), order="C")      
...
# The following  ensures continuity: previous break statements set in place to allow for final configuration is either ----------00 or  00-----------
    ...
    for i in range(1, int(It4) + 1): # +1 included to allow for full range in loop   
                ...
                DL =  HLMAT[i].shape[0]/2   # Refer to p. 1716; number of rows divided by 2        
                ...
                if HLMAT[i].shape[0] < 2* M[j]:   # it is 2*M instead of M beacuse you wish to compute 16x16 reduced density matrix before truncaiton
                            sd = HLMAT[i].shape[0]  
                else:
                            sd = 2*M[j]
